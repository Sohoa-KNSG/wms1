...
-- 1. Thêm lại cột gross_weight 
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.tbl_thung60_kho') AND name = 'gross_weight')
BEGIN
    ALTER TABLE dbo.tbl_thung60_kho ADD gross_weight DECIMAL(18,2) NOT NULL DEFAULT 0;
END
GO

-- 2. Cập nhật SP GetPalletInfo để tính trọng lượng TRỰC TIẾP TỪ KHO
