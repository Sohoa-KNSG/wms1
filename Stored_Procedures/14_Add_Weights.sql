...
-- 1. Thêm cột trọng lượng
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.tbl_thung60_kho') AND name = 'gross_weight')
BEGIN
    ALTER TABLE dbo.tbl_thung60_kho ADD gross_weight DECIMAL(18,2) NOT NULL DEFAULT 5.00; -- Giả định mỗi thùng 5kg
END
GO

IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.pallet') AND name = 'tare_weight')
BEGIN
    ALTER TABLE dbo.pallet ADD tare_weight DECIMAL(18,2) NOT NULL DEFAULT 15.00; -- Vỏ pallet 15kg
END
GO

-- 2. Cập nhật SP GetPalletInfo
...
BEGIN
    ...
    SELECT @TotalGrossWeight = ISNULL(SUM(gross_weight), 0)
    FROM AllCartons;
