...
### 2.2. Backend (ASP.NET Core - `PalletController.cs` & SQL)
- **API `GET /api/pallets/{id}/info`:**
  - C# không xử lý logic tính toán mà đẩy toàn bộ gánh nặng xuống SQL Server thông qua việc gọi Stored Procedure `usp_WMS_UC06_2_GetPalletInfo`.
...
```mermaid
flowchart TD
    A["Thủ kho Quét Mã Pallet"] -->|GET /info| B("API: Controller C#")
